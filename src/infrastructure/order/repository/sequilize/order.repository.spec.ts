...
describe("Order repository test", () => {
  ...
  it("should update a order", async () => {
    ...
    //Verify if the order was created
    const orderModel = await OrderModel.findOne({
      where: { id: order.id },
      include: ["items"],
    });
    ...
    const orderModeUpdated = await OrderModel.findOne({
      where: { id: order.id },
      include: ["items"],
    });

    expect(orderModeUpdated.toJSON()).toStrictEqual({
      id: "123",
      customer_id: "123",
      total: order.total(),
      items: [
  ...
  it("should find all orders", async () => {
    ...
    const list = await orderRepository.findAll();

    expect(list).toHaveLength(2);
    expect(list[0]).toEqual(order);
    expect(list[1]).toEqual(order2);
